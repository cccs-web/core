...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for project in orm.Project.objects.all():
            for feature_att, content_att in (
                    ('features', 'content'),
                    ('features_en', 'content_en'),
                    ('features_fr', 'content_fr'),
                    ('features_ru', 'content_ru')):
                features = getattr(project, feature_att)
                content = getattr(project, content_att)
                if features and features != content:
                    setattr(project, content_att, u"{content}\n{features}".format(
                        content=getattr(project, content_att),
                        features=features))
                    setattr(project, feature_att, '')
                    project.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        raise Exception('Cannot split features out of content')
