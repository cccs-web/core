...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for associate_role in orm.AssociateRole.objects.all():
            cccs_role = orm.CCCSRole(name=associate_role.name)
            for attr in ('name_en', 'name_fr', 'name_ru',
                         'plural_name_en', 'plural_name_fr', 'plural_name_ru'):
                setattr(cccs_role, attr, getattr(associate_role, attr))
            print(cccs_role.name, cccs_role.name_en)
            cccs_role.save()

        for cv in orm.CV.objects.all():
            if cv.associate_role:
                cccs_role = orm.CCCSRole.objects.get(name=cv.associate_role.name)
                cv.cccs_role = cccs_role
                cv.save()
